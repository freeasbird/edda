...
func route() (r *gin.Engine) {
	...
	controller.Secrets()
	// api 路由组
	api := r.Group("/edda/api/v1", gin.BasicAuth(gin.Accounts(config.CFG.Users)))

	// 授权码
	api.Any("/license/*id", controller.LicenseAPI)
	// 客户
	api.Any("/customer/*id", controller.CustomerAPI)
	// 应用
	api.Any("/app/*id", controller.AppAPI)
	// 节点/设备
	api.Any("/node/*id", controller.NodeAPI)
	// 序列号
	api.Any("/serial/*id", controller.SerialAPI)
